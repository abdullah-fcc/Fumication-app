// Shared shape + constants for the "Service Report" form, mirroring the
// Insta Fumigation & Pest Control Services daily report template
// (templates/Daily Service Report (General).pdf).

export type ReportType = 'routine' | 'follow_up' | 'complaint';

export interface PesticideEntry {
  name: string;
  rateLabel: string; // e.g. "ml/L", "gm/SFT" — empty for custom rows
  checked: boolean;
  rate: string;       // the value the worker fills in for the rate
}
...
export function actionLabel(value: string): string {
  return ACTION_OPTIONS.find((o) => o.value === value)?.label ?? '';
}
