  ...
  return (
    <div className="relative">
      ...
      >
        <Bell size={17} />
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-accent-600" />
        )}
      </button>
                ...
                notifications.map((n) => (
                  <button
                    ...
                    className={[
                      ...
                      n.is_read ? 'bg-white hover:bg-gray-50' : 'bg-brand-50/60 hover:bg-brand-50',
                    ].join(' ')}
                  >
                    <div className="flex items-start gap-2">
                      {!n.is_read && <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-brand-600 flex-shrink-0" />}
...
export default function Header({ title }: { title?: string }) {
  ...
  return (
    <header className="h-14 bg-white border-b border-gray-200 flex items-center px-4 sm:px-6 flex-shrink-0">
      {/* Brand lockup — the sidebar rail (which carries the logo) is hidden on phones */}
      <div className="flex items-center gap-2 sm:hidden">
        <LogoMark size={26} />
        <span className="text-sm font-bold text-brand-600 tracking-tight">{BRAND_NAME}</span>
      </div>
      {title && (
          ...
          <>
            ...
            <div className="absolute right-0 mt-1 w-44 bg-white rounded-lg border border-gray-200 shadow-lg py-1 z-50">
              <div className="px-3 py-2 border-b border-gray-100">
                ...
                <p className="text-xs font-medium text-brand-600 capitalize mt-0.5">{user?.role}</p>
              </div>
              <button
                onClick={logout}
